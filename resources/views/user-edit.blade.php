...
<li class="active"><a class="nav-link" href="{{route ('dashboard')}}"><i class="fas fa-tachometer-alt"></i>
...
@endif
<li><a class="nav-link" href="{{route ('tampil-transaksi')}}"><i class="fas fa-file-invoice-dollar"></i>
        <span>Transaksi</span></a></li>
@if (auth()->user()->role=="admin")
<li><a class="nav-link" href="{{route ('tampil-user')}}"><i class="fas fa-user-tie"></i> <span>Data Pengurus</span></a>
</li>
@endif
            ...
            <form action="{{route ('update-user', Auth::user()->id)}}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="form-group col-md-6 col-12">
                        <label>Nama</label>
                        <input type="text" class="form-control" name="name" @if (old('name')) value="{{old('name')}}"
                            @else value="{{$user->name}}" @endif>

                        <label @error('name') class="text-danger" @enderror>
                            @error('name')
                            {{$message}}
                            @enderror
                        </label>
                        ...
                        <div class="input-group mb-2 mr-sm-2">
                            ...
                            <input type="email" class="form-control" name="email" @if (old('email'))
                                value="{{old('email')}}" @else value="{{$user->email}}" @endif>
                        </div>
                        ...
                        <input type="password" class="form-control" placeholder="Kosongi password jika tidak diubah."
                            name="password" {{-- @if (old('password'))
                        value="{{old('password')}}" @else value="{{$user->password}}" @endif --}}>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="form-group col-md-5 col-12">
                        <label>Role</label>
                        <input type="text" class="form-control" name="role" @if (old('role')) value="{{old('role')}}"
                            @else value="{{$user->role}}" @endif disabled>
                    </div>

                    <div class="form-group col-md-5 col-12" hidden>
                        <label>Role</label>
                        <input type="text" class="form-control" name="role" @if (old('role')) value="{{old('role')}}"
                            @else value="{{$user->role}}" @endif>
                    </div>
                </div>
          </div>
